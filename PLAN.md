...
4. ~~**v2-geometrie**~~ ✅ *Gebouwd 2026-08-10* (`spike/s8_geometrie.py`): OSM-spoorgraaf uit Geofabrik-extracten (6 landen, 4,1 M knopen → 617k randen), stations gesnapt (≤1500 m), 18.658 van 20.626 baanvakken over het echte spoor gerouteerd (A*, tolerantie 2,2×; 572 fallback-rechte-lijnen). Output `paar_geometrie.json.gz` (2 MB) gaat via R2 naar de VM. ~~Herdraaien alleen nodig als het stationsbestand wezenlijk wijzigt.~~ *(2026-09-14: weerlegd — ook met een stabiele stationsset maakt één week feedwijzigingen honderden nieuwe clusterparen; `docs/verslag-2026-09-14.md`.)* *(2026-09-13: bekende defecten — A* kiest bij lange sprongen soms dood spoor, en de verfijning legt HSL-verkeer op de parallelle klassieke lijn — plus het plan om `shapes.txt` als bron te gebruiken: `docs/lijnvoering.md`.)*
...
5. Offline koppeling en classificatie, gevalideerd tegen bekende casussen: Cléon (11 sep) en de twee aangekondigde omleidingen uit `docs/hsl-omleidingen.md` (LGV Nord vanaf 14 sep, NBS Nürnberg–Ingolstadt vanaf 31 okt).
