...
Ranks German clusters in merged.duckdb by rail departures, keeps the busiest `--top`
ones, resolves each cluster name to an IRIS EVA number via the Timetables /station
endpoint, and validates that the EVA actually carries timetable data. Some hub EVAs are
...
REQUEST_INTERVAL_S = 1.1  # stay well under the 60 req/min plan limit
...
def main() -> None:
    ...
    ranked = con.execute(
        """WITH de_visits AS (
             SELECT sc.cluster_id, st.feed || ':' || st.trip_id AS trip
             FROM stop_times st
             JOIN stop_cluster sc USING (stop_id)  -- stop_times.stop_id is already feed-prefixed
             WHERE st.feed IN ('de_fv', 'de_rv', 'de_delfi')
           )
           SELECT c.cluster_id, c.naam, c.lat, c.lon, count(DISTINCT trip) AS n_trips
           FROM de_visits JOIN clusters c USING (cluster_id)
           GROUP BY 1, 2, 3, 4 ORDER BY n_trips DESC LIMIT ?""",
        [args.top],
    ).fetchall()
    con.close()
